...
const command_model = require("../models/commandModel.js")
const agent_model = require("../models/agentModel.js")
const ms = require("ms")
const { redis_client_presence } = require("../services/redis")
const { get_path, set_path, filter_object, create_stream } = require("../utils.js")
const { AGENT_HEARTBEAT_EXPIRY } = require("../configs/constants.js")

const agent_heartbeat_expiry = ms(AGENT_HEARTBEAT_EXPIRY)

module.exports.create_command = ({command_data_path = "command_data", agent_id_path = "agent_id", user_id_path = "user_id", output_command_data_path = "command_data"} = {}) => {
    return (req, res, next) => {
        ...
        if (command === undefined || agent_id === undefined || user_id === undefined) {
            console.error("No data found at path(s):", command_data_path, agent_id_path, user_id_path)
...
module.exports.get_command_by_agent_id_and_mark_sent = ({fields, agent_id_path = "agent_id", output_commands_path = "command_data"} = {}) => {
    return (req, res, next) => {
        const agent_id = get_path(res, agent_id_path)
...
module.exports.get_command_by_agent_id = ({fields, agent_id_path = "agent_id", output_command_data_path = "command_data"} = {}) => {
    return (req, res, next) => {
        const agent_id = get_path(res, agent_id_path)
        if (agent_id === undefined) {
            console.error("No data found at path(s):", agent_id_path)
...
module.exports.update_by_command_id = ({fields, command_id_path = "command_id", command_data_path = "command_data"} = {}) => {
    return (req, res, next) => {
        ...
        command_model.update_by_command_id({ command_id, ...command_data }, fields, (error, results) => {
            if (error) {
